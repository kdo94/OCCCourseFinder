...
class DBHelper extends SQLiteOpenHelper {
    ...
    public void addOffering(int crn, int semesterCourse, int courseId, int instructorId) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();

        values.put(OFFERINGS_KEY_FIELD_ID, crn);
        values.put(FIELD_SEMESTER_CODE, semesterCourse);
    ...
    public boolean importOfferingFromCSV(String csvFileName) {
        ...
        try {
            while ((line = buffer.readLine()) != null) {
                String[] fields = line.split(",");
                if (fields.length != 4) {
                    Log.d("OCC Offering Finder", "Skipping Bad CSV Row: " + Arrays.toString(fields));
                    continue;
                }
